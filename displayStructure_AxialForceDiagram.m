...
function displayStructure_AxialForceDiagram(bar,node,F_bar,Force_All)
    node_coordinate = displayStructure_original(bar,node);
    RESOLUTION = 0.01;

    % 计算尺度变换的大小
    axialForce_Scale = max(max(bar(:,4)))/4/max(max(abs(F_bar([1,4],:))));
    
    for i = 1:size(bar)
        axialForce_left  = F_bar(1,i);
        axialForce_right = -F_bar(4,i);

        x_local = 0:RESOLUTION:bar(i,4);

        % 使用两点式计算y坐标，bar(i,4)为杆件长度
        y_local = axialForce_left + (axialForce_right - axialForce_left)*x_local/bar(i,4);
        ...
        % 对y进行尺度变换
        y_local = y_local*axialForce_Scale;
