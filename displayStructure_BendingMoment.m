...
function displayStructure_BendingMoment(bar,node,F_bar,Force_All)
    node_coordinate = displayStructure_original(bar,node);
    RESOLUTION = 0.01;

    % 计算尺度变换的大小
    BendingMoment_Scale = max(max(bar(:,4)))/4/max(max(abs(F_bar([3,6],:))));
    
    for i = 1:size(bar)
        bendingMoment_left  = -F_bar(3,i);
        bendingMoment_right = F_bar(6,i);

        x_local = 0:RESOLUTION:bar(i,4);
        % 使用两点式计算y坐标，bar(i,4)为杆件长度
        y_local = bendingMoment_left + (bendingMoment_right - bendingMoment_left)*x_local/bar(i,4);

        for j = 1:size(Force_All,1)
            if Force_All(j,1) == 1 & Force_All(j,2) == i
                % 当前杆件存在分布力
                q = Force_All(j,4);
                y_local = y_local + q*x_local/2.*(x_local - bar(i,4));
            end
        end

        % 对y进行尺度变换
        y_local = y_local*BendingMoment_Scale;
